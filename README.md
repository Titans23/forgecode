...
- `plan`、`supervised`、`auto` 权限模式，支持会话/项目/用户规则、交互审批和 JSONL 审计；
...
M3 已把权限判断接入最终 ToolCall 参数与实际执行之间，并复用 M4 Session Journal 和 Checkpoint 作为唯一审计与回滚事实源。当前能够安全处理可信本机仓库中的常规任务；把任意恶意仓库视为完全隔离仍依赖后续 Windows/POSIX 原生 Sandbox 后端。
