...
当前项目处于早期开发阶段，但 M1 多步 Agent Loop、M2 工作区与完成门禁、M3 权限与安全控制核心、M4 会话恢复、M5 上下文工程以及 M6 Hooks 已经落地。本 README 同时描述当前可用能力、系统边界和从 M0 到 v1.0 的后续路线。
...
当前自动化测试基线为 `334 passed`。提交前建议同时运行：
...
不带参数时显示当前模式及用户、项目、会话规则数量。`plan` 只允许读取和分析，写入、Patch、命令与验证工具不会发送给模型；如果模型仍尝试调用隐藏工具，运行时会拒绝并立即结束本轮。`supervised` 对有副作用的操作发起交互审批，`auto` 自动放行低风险操作，但高风险和硬拒绝规则仍然生效。旧的 `/mode` 和 `/permissions` 命令不再提供。
