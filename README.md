...
M1.4 已形成可执行的 Agent Loop，但命令审批、敏感路径治理和更强的本机安全限制仍属于 M3；当前 Shell 写入检查只是行为护栏，不是安全沙箱。ForgeCode 当前也没有浏览器、截图或视觉理解能力，网页游戏只能进行源码、命令和测试层面的验证。
...
- [ ] 支持最大 Token 预算；
- [x] 在相同工作区版本中检测完全相同的工具和参数，失败调用不重复执行，成功调用最多执行两次；
- [x] 修改任务连续 8 次模型调用没有工作区或计划进展时提醒模型，连续 16 次无进展时以 `blocked` 安全结束；
- [ ] 保存完整 JSONL 执行轨迹；
- [x] 支持 `forge` 进入循环交互会话；
