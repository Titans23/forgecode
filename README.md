...
- **模型与运行时解耦**：路由模型负责自然语言意图、任务关系和验证需求；主模型负责行动与回答；运行时只负责确定性的执行证据、安全边界和结构化协议。
- **结构化失败**：工具、命令和模型调用失败时返回结构化错误，不让 Agent 进程直接崩溃。
- **安全默认值**：仓库外访问、高风险命令、网络和敏感文件默认受限。
- **全程可追踪**：模型响应、工具调用、权限判断、文件修改和验证结果均进入事件轨迹。
- **可恢复而非重来**：当前目标、工作证据和可压缩消息历史分层保存；持久化会话、代码 Checkpoint、`/undo`、`/rewind`、回放和冲突保护已经接入。
...
自动化测试之外，涉及路由、权限、恢复或上下文的改动还应直接驱动完整 `Conversation → Router → ToolRegistry → WorkspaceTracker → finish_task` 链路，在临时 Git 仓库中发送真实用户措辞并检查模型可见工具、任务状态、磁盘结果和结束状态。Fake 单元测试用于稳定复现边界，但不能替代这种端到端实战；调用真实 Provider 的 CLI 验收会发送 Prompt 与项目上下文并产生费用，应在明确授权后单独执行。
...
M0 的三个 Fixture 仅用于项目维护者评测。普通用户无需同时安装这些 Fixture 的全部语言工具链。M3 已实现命令审批、仓库路径限制、敏感目标保护和子进程约束；CPU、内存、进程数量以及原生文件系统/网络强隔离仍由后续平台 Sandbox Backend 补齐。Docker 如有需要，只作为 M7 Benchmark 的可选执行后端。
...
不带参数时显示当前模式及用户、项目、会话规则数量。`plan` 只允许读取和分析，写入、Patch、命令、验证和 MCP 工具不会发送给模型；如果模型仍尝试调用隐藏工具，运行时会拒绝并立即结束本轮。`supervised` 对有副作用的操作发起交互审批，`auto` 自动放行低风险操作，但高风险、MCP 外部调用和硬拒绝规则仍然生效。旧的 `/mode` 和 `/permissions` 命令不再提供。
...
里程碑编号表示能力主题，不再强制表示实施顺序。当前 M1～M6 的核心能力已经进入代码库；M3 的原生文件系统/网络强隔离仍按平台后端继续加固，M7 负责 Benchmark、消融实验与 v1.0 验收。
...
- [x] 当前 revision 已有真实 Diff 且满足任务策略后向模型提供客观 Completion Evidence；只有路由决策或显式 `TaskPolicy(require_verification=True)` 要求验证，变更任务通过结构化 `finish_task` 声明完成，不扫描自然语言中的“完成/未完成”短语。
...
### 当前状态：权限、审批、审计、进程安全与代码回滚核心完成；原生文件系统/网络强隔离继续加固

M3 在两层执行权限约束：模型请求前按照当前模式裁剪可见工具，工具实际执行前再基于最终 ToolCall 参数进行风险分类和授权。任何权限拒绝都会终止当前轮次，不进入 Edit Recovery 或继续消耗模型调用；同批未执行工具只写入协议配对结果，保证会话能够合法恢复。权限审计和代码回滚复用 M4 Session Journal 与 Checkpoint 作为唯一事实源。当前能够安全处理可信本机仓库中的常规任务；把任意恶意仓库视为完全隔离仍依赖后续 Windows/POSIX 原生 Sandbox 后端。
...
- [x] `plan`：只读和分析，不修改文件；
- [x] `supervised`：代码编辑和高风险命令需要确认；
- [x] `auto`：低风险行为自动执行，高风险行为仍需确认；
- [x] 使用唯一入口 `/permission [plan|supervised|auto]` 查看或切换模式；Plan 模式在模型请求前隐藏写入和进程工具；
- [x] 权限拒绝会立即终止当前轮次，不再进入 Edit Recovery 或重复调用模型；
- [x] 支持 `allow`、`ask` 和 `deny`，并定义 deny 优先、具体规则优先和 scope 优先级；
- [x] 支持用户级、项目级和会话级规则；
- [x] 支持“允许一次”“本会话允许”“当前项目允许”和“拒绝”，拒绝原因以结构化结果反馈给 Agent。

文件安全：

- [x] 标准化所有路径，阻止 `../`、绝对路径和软链接路径逃逸；
- [x] 默认禁止访问仓库外路径；
- [x] 默认保护 `.env`、SSH 私钥、常见凭证、`.forge` 和 `.git` 内部文件；
- [ ] Patch 大小已有硬限制；跨文件数量上限继续在 Patch 解析器中补齐。

Shell 安全：

- [x] 设置命令超时和 stdout/stderr 长度限制，超时终止整个进程树；
- [x] 清理 API Key、Token、密码等敏感环境变量并禁止常见提权命令；
- [x] 网络访问、依赖安装和文件删除单独审批，破坏性 Git 恢复硬拒绝；
- [ ] 后台进程登记、查询和主动终止继续由 Process Registry 补齐；
- [ ] CPU、内存、进程数量以及文件系统/网络强隔离继续由平台 Sandbox Backend 补齐。

Checkpoint：

- [x] Agent 每个用户回合第一次修改前创建内容寻址 Checkpoint；
- [x] 保存并通过 after-hash 冲突检查保护用户原有的未提交修改；
- [x] 每轮实际修改记录 Checkpoint 和会话节点；
- [x] `/undo` 恢复最近一个包含文件快照的 Agent 修改；
- [x] `/rewind` 按 code、conversation 或 both 恢复指定任务节点；
- [x] 回滚不依赖 `git reset --hard`；
- [x] WorkspaceTracker 区分用户基线和 Agent 修改；
- [x] Shell 命令造成的文件变化单独标记为 process mutation；未知目标 Shell 写入的完整前镜像仍依赖原生 Sandbox/overlay 后端。
...
当前 M3 核心适合可信本机仓库和受监督的高风险操作；只有平台原生文件系统、网络与资源 Sandbox Backend 完成后，ForgeCode 才进入任意不可信大型仓库的试用阶段。
...
参考 Claude Code 的公开会话模型，让任务不会因为进程退出而丢失，并明确区分两个相互独立的能力：

- **会话恢复**：持久化对话、工具调用与结果、任务状态和上下文压缩结果；
- **文件 Checkpoint**：保存 ForgeCode 文件编辑工具修改前的内容，用于会话级撤销，不替代 Git。

M4 首版优先保证可靠恢复和不重复执行工具，不尝试自动恢复整个 Git 工作区。

### 任务

会话存储采用 append-only JSONL 作为唯一事实来源：

- [x] 在用户目录下按项目保存会话：`~/.forge/projects/<project-key>/sessions/<session-id>.jsonl`，不污染任务仓库和 Git 状态；
- [x] 记录 `SessionStarted`、`UserMessage`、`AssistantMessage`、`ToolRequested`、`ToolStarted`、`ToolResult`、`TaskState`、`ContextCompacted`、`TurnCompleted`、`SessionStopped` 和 `SessionResumed`；
- [x] 每个事件包含 UUID、父事件 UUID、会话 ID、递增序号和时间戳；
- [x] 完整保存恢复所需的模型消息、工具参数和工具结果，保证 `tool_use` 与 `tool_result` 协议配对合法；
...
- [x] `--continue` 选择当前项目最近的可恢复会话，`--resume` 按 ID 或名称选择会话；
- [x] 从最后一个完整、协议合法的事件边界恢复，忽略崩溃留下的不完整 JSONL 尾部或消息片段；
- [x] 恢复完整对话、模型配置、活动任务、上下文摘要和最近验证结果；活动任务的单轮计时、Token 和恢复次数重新计数；
...
- [x] 每个用户提示建立一个 Checkpoint，并在 `write_file`、`apply_patch`、`replace_text`、`replace_line_range` 和分块写入首次修改目标文件前保存原内容；
- [x] 使用 SHA-256 内容寻址 Blob 保存原文件，记录文件是否原本存在；
- [x] 回滚时恢复被修改文件，并删除由 ForgeCode 编辑工具新建的文件；
- [x] 不把 Checkpoint 宣称为 Git 或系统级快照；Shell 命令、外部程序、符号链接和并发会话造成的修改仅检测并警告，首版不自动回滚；
- [x] 恢复前校验目标文件当前哈希，发现用户或外部修改时停止覆盖并请求确认；
- [x] 为 Checkpoint 设置数量和时间保留策略，清理不再被引用的 Blob。

索引与命令：

- [x] 使用可重建的 `index.json` 加速会话列表，JSONL 始终是事实来源；首版不引入 SQLite 双重状态；
- [x] 支持 `forge --continue`、`forge --resume [<id-or-name>]`、`forge sessions` 和 `--fork-session`；
- [x] 支持 `/resume`、`/rename`、`/history`、`/status`、`/branch`、`/rewind`、`/compact` 和 `/clear`；
- [x] 输入 `/resume` 打开非全屏内联会话选择器并原地切换上下文，支持方向键、Enter、Esc 和 Ctrl+C；Slash 补全支持会话命令，并在 `/resume ` 后动态补全当前项目的会话名和 ID；
- [x] `/rewind [checkpoint-id] [code|conversation|both]` 可分别选择恢复对话、恢复代码或同时恢复两者。

### 验收条件

- 任务运行中强制结束进程后，可通过 `forge --continue` 恢复；
- 不重复执行已经成功的工具调用；
- JSONL 尾部损坏或模型流式响应中断时，仍能恢复到最后一个合法边界；
- 可以查看恢复前的计划、修改、测试结果和上下文压缩摘要；
- `/rewind` 能恢复 ForgeCode 编辑工具造成的文件修改，并在外部文件变化时拒绝静默覆盖；
- 可以从旧会话创建新会话分支尝试不同方案，原会话保持不变。
...
Hook 配置从用户级 `~/.forge/settings.json` 和项目级 `.forge/settings.json` 加载，按用户配置在前、项目配置在后的顺序合并。每个 Hook 使用可选的工具和仓库相对路径 Glob 进行匹配；命令必须写成参数数组，并通过无 Shell 的子进程执行。命令在仓库根目录运行，通过 stdin 接收事件 JSON，同时可以读取 `FORGE_HOOK_EVENT`、`FORGE_SESSION_ID`、`FORGE_TOOL_NAME` 和 `FORGE_FILE_PATH` 环境变量。`{event}`、`{session_id}`、`{tool}` 和 `{path}` 可以作为完整命令参数中的占位符。
...
退出码 `0` 表示放行，退出码 `2` 表示明确拒绝，其余非零退出码表示执行错误。Hook 也可以在 stdout 返回 JSON，通过 `decision` 放行或拒绝，通过 `updated_arguments` 修改工具参数，通过 `additional_context` 注入上下文，并用 `reason` 解释决策。

```json
{
  "decision": "allow",
  "updated_arguments": {"path": "src/app.py"},
  "additional_context": "该文件由代码生成器维护。",
  "reason": ""
}
```

`PreToolUse` 和 `BeforeFileEdit` 修改后的参数会重新经过工具参数校验、重复调用检查、Checkpoint 和实际执行；拒绝会转成模型可见的 `hook_denied` 工具结果。`SessionStart` 或工具后置 Hook 注入的上下文只进入下一次模型请求，`BeforeModelCall` 注入的内容只进入当前请求，不会污染持久化的原始消息历史。

操作前 Hook 在超时、非法 JSON、启动失败或非零错误退出时采用 fail-closed；操作后 Hook 无法撤销已经完成的操作，因此记录错误但保持原操作结果。命令输出总量限制为 1 MB，审计预览限制为 2000 个字符。每次执行都会以 `hook_execution` 事件写入当前 Session JSONL，记录事件、耗时、决策、退出码、参数是否修改和有限输出预览，不保存完整模型上下文或完整工具参数。

项目 Hook 等同于在本机执行项目提供的程序。只应在可信仓库中启用或提交 `.forge/settings.json`，并避免在 Hook 输出中打印密钥和其他敏感信息。

已覆盖编辑 Python 后格式化、禁止修改特定目录、测试通过后通知，以及通过匹配 `finish_task` 在完成前运行安全扫描等场景。

### 6.2 MCP Client

ForgeCode 作为 MCP Host 管理长期存活的 MCP Client，而不是把 MCP 简化成普通 HTTP 调用。交互 CLI 已改为整个进程只使用一个 Event Loop，使 Transport、ClientSession 和通知任务能够跨用户回合安全存活并按顺序关闭。当前锁定稳定版 `mcp>=1.28,<2`，避免 SDK v2 发布前的预发布破坏性变化。

- [x] 实现 MCP Client Manager 和连接状态管理；
- [x] 支持 stdio Transport 和 Streamable HTTP；
- [x] 支持初始化、能力协商、分页 `tools/list` 和 `tools/call`；
- [x] 支持 `notifications/tools/list_changed`、连接/调用超时和断线恢复；
- [x] 将 MCP Tool 以 `mcp__<server>__<tool>` 映射到动态统一 Tool Registry；
- [x] MCP Server 连接和 MCP Tool 调用经过 ForgeCode 权限系统，且不信任 Server 的 Tool Annotation 作为安全事实；
- [x] Plan 模式不连接 MCP Server，并从模型请求中隐藏已发现的 MCP Tool；
- [x] 工具、连接和刷新事件在 Session JSONL 中标记 MCP 来源、Server 和 Transport；
- [x] 工具调用断线返回 `mcp_result_unknown`，重新连接时不自动重放可能已有副作用的调用；
- [x] 提供 `/mcp` 状态命令、本地 echo/add 示例 Server 和 stdio/HTTP 真实集成测试；
- [x] 首版不自行实现 OAuth、旧 SSE、Resources、Prompts、Sampling 和完整远程认证体系。

### 6.3 Explore Subagent

首个子 Agent 只负责只读仓库探索，可使用 `list_directory`、`find_files`、`grep`、`read_file` 和 `git_log`。主 Agent 通过普通的 `explore_repository` 工具发起委派；Explore Agent 复用 Agent Loop，但使用全新的模型客户端、消息历史和只读 Tool Registry，不会继承主会话上下文，也不能再次委派。

Explore Agent 默认最多执行 8 轮，独立累计输入预算为 120000 Token。原始文件内容、搜索结果和中间工具轨迹都只保留在子上下文，返回主 Agent 前必须通过严格的结构化报告校验；主上下文只接收摘要、相关文件、调用路径、根因假设、建议修改点、不确定问题以及独立的 Token/轮次统计。主 Agent 可以继续核验或忽略这份报告。

- [x] 使用独立上下文和独立 Token Budget；
- [x] 设置最大执行轮数；
- [x] 仅拥有只读权限，不能修改文件；
