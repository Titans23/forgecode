    ...
    def _run_command(
        ...
        *,
        ...
        return (
            ...
            'FORGECODE_TIMED_OUT=false; '
            'if [ "$FORGECODE_EXIT" -eq 124 ]; then '
            '  FORGECODE_TIMED_OUT=true; '
            'fi; '
            'printf \'{"exit_code":%s,"timed_out":%s}\\n\' '
            '  "$FORGECODE_EXIT" "$FORGECODE_TIMED_OUT" '
            '  > /logs/agent/forgecode-status.json.tmp; '
            'mv /logs/agent/forgecode-status.json.tmp '
            '  /logs/agent/forgecode-status.json; '
